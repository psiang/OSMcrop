...
def getpic(y2, x1, y1, x2, z, source='google', outfile="MAP_OUT.png", style='s'):
    '''
...
def my_file_out(zb, file, target="keep", output="file"):
    '''
    zh_in  : tile coordinate
    file   : a text file for ArcGis
    target : keep = tile to Geographic coordinate
    ...
    x_patch_num = zb["RB"][0]-zb["LT"][0] + 1
    y_patch_num = zb["RB"][1] - zb["LT"][1] + 1
    ...
    x_r = (max_x-min_x)/(256.0*x_patch_num)
    y_r = (min_y-max_y)/(256.0*y_patch_num)
...
if __name__ == '__main__':
    # google amap tencent
    x = getpic(30.525773, 114.347530, 30.536521, 114.360538,
               17, source='google', style='s', outfile="google_17m_whuxx.tif")
    my_file_out(x,"google_17m_whuxx.tfw","keep")
